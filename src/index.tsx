...
type SessionMeta = { prompt: string; title: string }

const EMPTY_META: SessionMeta = { prompt: "", title: "" }
...
const readFirstPromptFromBuffer = (buf: Buffer): string => {
...
const readTitleFromBuffer = (buf: Buffer): string => {
...
const readSessionMetaFromBuffer = (buf: Buffer): SessionMeta => ({
...
const runBanner = async (ready?: Promise<unknown>) => {
...
if (process.argv[2] === "clean") {
  ...
  while (true) {
    ...
    if (firstLaunch && !process.argv.includes("--no-banner")) {
      ...
      await runBanner(sessionPreload)
      firstLaunch = false
    }
